...
const PACKAGE_DIR = join(ROOT, "backend");
const PACKAGE_NAME = "@retinue/agentkit";

/**
 * Deep imports that must be refused, each chosen for a way somebody would actually reach.
 *
 * `dist/index.js` is the honest shortcut — the file is right there and it works in the workspace. `src/…` is what
 * an editor's auto-import offers while the platform sits next to the runtime in one checkout. `internal` is the
 * name a codebase gives a private entry once someone has asked for one. `adapters` is the half-path: a real
 * subpath is `./adapters/postgres`, and the parent must not resolve to anything.
 */
...
export const exportedSubpaths = (manifest) =>
  Object.keys(manifest.exports ?? {})
    // `./package.json` is exported on purpose — tooling reads it — but it is not a module to import.
    .filter((subpath) => subpath !== "./package.json")
    .map((subpath) => (subpath === "." ? PACKAGE_NAME : `${PACKAGE_NAME}/${subpath.replace(/^\.\//, "")}`));
 ...
 * that reappears when someone widens a glob, a `.map` that ships the source text by another route.
 */
...
// ── the artifact ──────────────────────────────────────────────────────────────────────────────────────────────
  ...
  if (!existsSync(join(PACKAGE_DIR, "dist", "index.js"))) {
    die(`${PACKAGE_DIR}/dist is not built — run \`npm run build\` first; this check is about the shipped artifact`);
  }

  const manifest = JSON.parse(readFileSync(join(PACKAGE_DIR, "package.json"), "utf8"));
  const subpaths = exportedSubpaths(manifest);
  if (subpaths.length < 2) die("the manifest declares fewer than two subpaths — the exports map is not what this expects");
  ...
  try {
    let packed;
    try {
      const output = run("npm", ["pack", "-w", PACKAGE_NAME, "--pack-destination", work, "--json"], { cwd: ROOT });
      packed = join(work, JSON.parse(output)[0].filename);
    } catch (error) {
      die(`npm pack failed: ${error.stderr || error.message}`);
    }
    ...
    if (forbidden.length > 0) {
      fail(
        `the tarball ships ${forbidden.length} file(s) it should not: ${forbidden.slice(0, 5).join(", ")}`,
        "sources and sourcemaps make the internals readable, and readable internals become the API whichever\n" +
          "subpaths the exports map declares",
      );
    }

    // ── a consumer that knows the package only through node_modules ──────────────────────────────────────────────
    const consumer = join(work, "consumer");
    const modules = join(consumer, "node_modules");
    mkdirSync(join(modules, "@retinue"), { recursive: true });
    run("tar", ["-xzf", packed, "-C", join(modules, "@retinue")]);
    run("mv", [join(modules, "@retinue", "package"), join(modules, "@retinue", "agentkit")]);
    ...
    writeFileSync(
      ...
      `const attempt = async (specifier) => {\n` +
        ...
        `const deep = ${JSON.stringify(DEEP_IMPORTS)};\n` +
    ...
    let probe;
    try {
      probe = JSON.parse(run(process.execPath, [join(consumer, "probe.mjs")], { cwd: consumer }));
    } catch (error) {
      die(`the consumer probe did not run: ${error.stderr || error.message}`);
    }
    ...
    for (const outcome of probe.deep) {
      if (outcome.loaded) {
        fail(
          ...
          "a consumer can reach an internal module, so it is public API in practice and every refactor behind it\n" +
            "is a breaking change nobody will describe as one",
        );
        ...
        fail(
          ...
          "it was not the boundary that stopped it. That distinction is the guarantee: a missing file stops being\n" +
            "missing the moment somebody adds it, and then nothing is left refusing the import",
        );
      }
    }

    // ── 2 & 4: the same thing at build time, which is the form the platform's CI would hit ───────────────────────
    ...
    if (!legal.ok) {
      fail(
        "a consumer cannot typecheck against the exported subpaths",
        legal.output.trim() ||
    ...
    for (const specifier of DEEP_IMPORTS) {
      const deep = typecheck("deep", `import * as reached from "${specifier}";\nexport const names = Object.keys(reached);\n`);
      if (deep.ok) {
        fail(
          `${specifier} typechecks from a consumer — the build would not stop the platform reaching in`,
          "the runtime half is refused at import and the build says nothing, so the deep import is written, merged,\n" +
    ...
    if (ok) {
      console.log(
        `✓ ${PACKAGE_NAME} boundary holds as installed: ${subpaths.length} subpaths load and typecheck,` +
          ` ${DEEP_IMPORTS.length} deep imports refused by the exports map, ${entries.length} files shipped` +
          ` with no sources or sourcemaps`,
      );
    }
  } finally {
    cleanup();
