...
| `tools-google/*` | `tools-google` | Mail and calendar writes are **directed**, not broadcast. A message goes to addresses the caller named and an invitation to attendees it listed — the same reasoning as the WhatsApp sends below. A large distribution list is still a list of recipients, not a place strangers find things. Six outward writes, one reason |
...
| `tools-google` | **14 built** (Gmail and Calendar) · `gmail_search_messages`, `gmail_get_message`, `gmail_get_thread`, `gmail_list_labels`, `gmail_send_message`, `gmail_reply_message`, `gmail_create_draft`, `gmail_modify_labels`, `calendar_list_events`, `calendar_get_event`, `calendar_find_free_time`, `calendar_create_event`, `calendar_update_event`, `calendar_delete_event` (`destroys`). Gmail's scopes are Google-**restricted**, so they carry an app-verification burden. `gmail_create_draft` is deliberately ungated — see the integration page. Drive/Docs/Sheets remain specified | productivity | [#234](https://github.com/Rise-Experts/retinue/issues/234), [#235](https://github.com/Rise-Experts/retinue/issues/235) |
